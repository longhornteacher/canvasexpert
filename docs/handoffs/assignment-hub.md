...
**GREEN** — all acceptance criteria hold, both required test gates pass, the final live
smoke verified restriction and both tag overrides, and the teacher removed the test
objects. Ready to commit and retire this brief.

- Commit: none; senior owns commit. HEAD remains `0a66fda6312d2b1edb42319dab666c78645963f6`.
- Changed files: `api/README.md`, `api/content_push.py`, `api/default_docs/AI Authoring/Author an Assignment (AssignmentForge).txt`, `api/mcp_server/tools.py`, `api/operation_ledger/adapters/assignment.py`, new `api/operation_ledger/adapters/assignment_hub.py`, `api/operation_ledger/adapters/differentiated_bridge.py`, `api/operation_ledger/catalog_reconcile.py`, `api/tests/mcp_server/test_content_push_tools.py`, new `api/tests/test_assignment_hub_operation.py`, `api/tests/test_assignment_operation.py`, `api/tests/test_assignment_tier_operation.py`, `api/tests/test_operation_ledger.py`, `api/tests/webui/test_af.py`, `api/webui/af.py`, `docs/contracts/canvas-transport-owners.json`, `docs/handoffs/assignment-hub.md`, `docs/mcp-server.md`, `docs/reference/mutation-reconciliation-map.md`, `docs/reference/operation-ledger-module-map.md`, `engine/rendering/forge/canvas_html.py`, `engine/tests/rendering/forge/test_canvas_html.py`.
- Defect fixed: `/date_details` restriction, group assignment, and clear requests now put `only_visible_to_overrides` and `assignment_overrides` at the request root. The fake asserts the flat shape and rejects a nested `wiki_page` body.
- Verification: brief focused gate (`py -m pytest -p no:randomly engine/tests api/tests/webui/test_af.py api/tests/test_assignment_operation.py api/tests/test_assignment_tier_operation.py api/tests/test_assignment_ordered_steps.py api/tests/test_differentiated_bridge.py api/tests/test_page_operation.py api/tests/test_canvas_mutation_ownership.py api/tests/test_operation_ledger.py api/tests/mcp_server`) — 546 passed, 2 warnings, 70.06s. `py -m pytest -p no:randomly api/tests` — 2046 passed, 5 warnings, 186.31s. Targeted Hub/ownership — 26 passed, 2 warnings, 4.20s. `git diff --check` passed.
- Earlier senior review repairs: freeze tag resolution at preparation; checkpoint and verify separate compensation steps for unpublishing and clearing group overrides. Resume with changed groups and unreadable visibility state now fails closed; regression tests cover these paths.
- Live smoke: after the teacher removed the preceding smoke objects and restarted Desktop with the endpoint correction, preview showed two matched tags and no publish/module/SIS effects. Apply completed successfully with both tag overrides and no teacher actions. `verify_live` confirmed the assignment and both tier pages were unpublished and outside modules; the operation result confirmed both tiers remained matched. After the teacher removed these three test objects, exact-title `verify_live` checks returned `found: false` for the assignment and both pages.
- Defect fixed in this follow-up: `_assign` reads `GET /api/v1/groups/{group_id}` and verifies returned `id`, frozen `group_category_id`, normalized tag name, and `non_collaborative: true` before setting an override. The fake exercises the exact route; regression cases reject a wrong category, changed name, or mismatched group ID while preserving fail-closed clearing.
- Deviations: four synthetic Canvas smoke attempts were needed to find and correct the two live API request issues. The final attempt created one unpublished assignment and two unpublished pages with no module placement or SIS posting. No local Web UI or MCP server was started. Untracked `stubbed-workspace/` remains untouched and unstaged.
- Unresolved decisions: none. Commit and retire this brief in the same batch.
