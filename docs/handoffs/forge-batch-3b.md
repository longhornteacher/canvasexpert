...
- Corrective review commit: `22b5244` (`Restore Forge MCP guidance and tool details`); restored the omitted scoring-session resume, feedback-contract, stale-takeover, text-limit, and stage/apply/resume refusal guidance while keeping the existing 2,200-character instruction and 18,569-character tool-list budgets. Restored the full stage_content service docstring and added safe attachment/result details to the new tool.
...
- Corrective verification: the named focused gate reran after `22b5244` — 736 passed in 76.32s; the full API gate — 2,024 passed, 5 warnings in 216.36s. The v62 live-contract generator was rerun under pytest; it produced no file diff because the snapshot records parameter shape, not tool prose.
