...
private scoring engine. Canvas Expert has no hosted grader. The agent scores only
the SAFE pseudonymized packet, then submits results to Canvas Expert for validation
and the exact-session Canvas write.
...
Ordinary assignments and New Quizzes use the same public start -> packet -> submit
flow. The server selects the transport privately. Ordinary assignments retain the
...
edit. Canvas Live is the review/edit surface. Canvas Expert has no local approval
