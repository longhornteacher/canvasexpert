...
Public teacher flow: MCP `start_scoring_session` -> `continue_scoring_session` ->
`get_scoring_packet` -> `submit_scoring_results`. One root session advances through
a frozen assignment queue; packets and writes remain assignment-bounded. Canvas Live
is the review/edit surface.
